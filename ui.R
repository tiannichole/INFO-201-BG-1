...
ui <- fluidPage(
  ...
  navbarPage("INFO 200 BG-1",
          ...
          know about licensing regulations simply choose to ignore them.", 
             ...
             tabPanel("Overview of our data",
                      sidebarLayout(
                        sidebarPanel(
                          h2("Choose what to examine:"),
                          #graph inputs here
                          dateRangeInput("dateRange", "Select Date Range:",
                                         start = min(pet_df$License.Issue.Date),
                                         end = max(pet_df$License.Issue.Date),
                                         min = min(pet_df$License.Issue.Date),
                                         max = max(pet_df$License.Issue.Date)),
                          selectInput("speciesInput", "Select Species:",
                                      choices = c("All", unique(pet_df$Species)),
                                      selected = "All"),
                          h5("In these graphs, each datapoint represents a pet license. 
                    The X axis reflects the time of issue, while the Y axis shows
                    the Annual Gross Income (AGI) of the pet owner. As you can see, we
                    have the most data for dogs, then cats, and very few licenses
                    for pigs or goats. From our data, AGI doesn't seem to have a
                    relationship with pet species.")
                          
                        ),
                        mainPanel(
                          h1("Pet licenses: relationships between species, socioeconomic status of owners, and year"
                          ),
                          plotOutput("plot1")
                        )
                      )
             ...
             tabPanel("Wealth and Species",
                      sidebarLayout(
                        sidebarPanel(
                          h2("Options"),
                          #graph inputs here
                          selectInput("species_var", "Select Species Variable:",
                                      choices = c("Dog", "Cat"),
                                      selected = "Dog"),
                          
                          # Input for selecting SES variable
                          selectInput("SES_var", "Select SES Variable:",
                                      choices = c("Low", "Medium", "High"), 
                                      selected = "Low"),
                          h5("As you can see, the proportion different breeds of dogs and cats change depending on the SES of owners.")
                        ),
                        mainPanel(
                          # plotlyOutput(outputId = "vis3_plot")
                          plotOutput(outputId = "vis3_plot")
                        )
                      )
             ),
             tabPanel("Conclusion",
                ...
                the project's focus:", 
                      ...
                      p("According to the data, there is a relationship 
